...
RSpec.describe ProposalMailer, type: :mailer do
  ...
  let(:advertiser) { create(:advertiser, name: "Garagem Trilha Livre", email: "contato@trilhalivre.com.br") }
  let(:listing) { create(:listing, title: "Jeep Wrangler Rubicon", advertiser: advertiser) }
  let(:proposal) { create(:proposal, listing: listing, amount_cents: 33_000_000, email: "comprador@exemplo.com.br") }

  it "vai para o anunciante" do
    expect(mail.to).to eq([ advertiser.email ])
  end
  ...
  it "cumprimenta o anunciante pelo nome" do
    expect(mail.text_part.decoded).to include(advertiser.name)
  end

  # Mailer não herda os helpers da aplicação como os controllers herdam.
  # Sem o `helper ListingsHelper` no ApplicationMailer, isto estoura.
  ...
  it "leva o link do anúncio" do
    expect(mail.text_part.decoded).to include(listing.id.to_s)
  end

  it "manda também a versão HTML" do
    expect(mail.html_part.decoded).to include(listing.title)
  end
end
