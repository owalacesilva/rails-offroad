...
RSpec.describe Advertiser, type: :model do
  ...
  describe "normalização de e-mail" do
    it "guarda em minúsculas e sem espaços" do
      advertiser = create(:advertiser, email: "  CONTATO@Trilha.com.BR ")

      expect(advertiser.email).to eq("contato@trilha.com.br")
    end

    it "encontra o registro por e-mail não normalizado" do
      advertiser = create(:advertiser, email: "contato@trilha.com.br")

      expect(described_class.find_by(email: "CONTATO@TRILHA.COM.BR")).to eq(advertiser)
    end
  end
  ...
  describe "telefone" do
    it "aceita entrada formatada, porque a normalização roda antes da validação" do
      expect(build(:advertiser, phone: "(41) 98877-0011")).to be_valid
    end

    it "recusa número curto demais" do
      expect(build(:advertiser, phone: "1234")).not_to be_valid
    end
  end

  describe "autenticação" do
    let!(:advertiser) { create(:advertiser, email: "contato@trilha.com.br", password: "trilha123") }

    it "autentica com a senha correta" do
      expect(described_class.authenticate_by(email: "contato@trilha.com.br", password: "trilha123")).to eq(advertiser)
    end
    ...
    it "não guarda a senha em texto puro" do
      expect(advertiser.password_digest).not_to include("trilha123")
    end
  end

  describe "#location" do
    it "junta cidade e UF" do
      expect(build(:advertiser, city: "Cuiabá", state: "MT").location).to eq("Cuiabá, MT")
    end
  end
end
