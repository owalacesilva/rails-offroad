...
RSpec.describe "Sessões", type: :request do
  let!(:advertiser) { create(:advertiser, name: "Garagem Trilha Livre", email: "contato@trilha.com.br") }
  ...
  describe "POST /entrar com credenciais corretas" do
    it "cria a sessão no banco" do
      expect { sign_in(advertiser) }.to change(Session, :count).by(1)
    end

    it "prende a sessão ao anunciante" do
      sign_in(advertiser)

      expect(Session.last.advertiser).to eq(advertiser)
    end
    ...
    it "passa a mostrar o nome no header" do
      ...
      expect(response.body).to include(advertiser.name)
    end
  end

  describe "POST /entrar com credenciais erradas" do
    it "não cria sessão" do
      expect { sign_in(advertiser, password: "errada") }.not_to change(Session, :count)
    end

    it "responde 422" do
      sign_in(advertiser, password: "errada")
